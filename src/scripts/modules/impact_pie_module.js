...
function create_impact_pie() {
        ...
        var impact_index = {
            "Environemnt_Energy": 0,
            "Social": 1,
            "Food_Quality": 2
        }
        // Create dummy data
        var data = {
            "Environemnt_Energy": 25,
            "Social": 25,
            "Food_Quality": 25
        };
        ...
        var label_angle_gap_to_avoid_overlapping = Math.PI / Object.keys(memorize_label_count_per_year).length / estimating_labels_per_impact_per_year;
...
function get_impact_arch_angles_from_pie(data_ready,impact_index,d) {
    ...
    if(all_measurement.length ==1) {
        var impact = name_dict[all_measurement[0]];
    ...
    if(all_measurement.length ==3) {
        ...
        if(impact_index[name_dict[all_measurement[0]]] + impact_index[name_dict[all_measurement[1]]] == 2) {
            return [data_ready[2].startAngle - 2* Math.PI,data_ready[0].endAngle]
        } else if(impact_index[name_dict[all_measurement[0]]] + impact_index[name_dict[all_measurement[1]]] == 1) {
